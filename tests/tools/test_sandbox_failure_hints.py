...
class TestSandboxFailureHint:
    ...
    def test_helper_import_failure_reports_module_skew(self):
        ...
        assert "stale" in h
        assert "BUILT-IN" not in h
        assert "no import" not in h.lower()

    def test_helper_name_error_hint(self):
        for helper in ("json_parse", "shell_quote", "retry"):
            err = f"NameError: name '{helper}' is not defined"
            h = _sandbox_failure_hint(err)
            assert h == f"Import {helper} before calling it: from hermes_tools import {helper}"
